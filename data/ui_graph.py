...
class Interaction(Data,Graph):
    def __init__(self, conf, training, test):
        ...
        self.ui_adj,self.train_mat = self.__create_sparse_bipartite_adjacency()
        self.norm_adj,self.ai,self.aj = self.normalize_graph_mat(self.ui_adj)
    ...
    def __create_sparse_bipartite_adjacency(self, self_connection=False):
        ...
        for i  in range(len(user_np)):
            train_mat[user_np[i],item_np[i]] = 1.0
            
        print(len(user_np))
        return adj_mat,train_mat
